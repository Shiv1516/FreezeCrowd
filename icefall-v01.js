...
function createSnowLake() {
  ...
  const snowFlake = `<div class='flake-wrapper' style='width:40px; left:${flakePos}%;height:40px;transform:scale(${flakeSize});animation:falling ${fallingTime}s linear infinite'></div>`;

  const backgroundElement = document.querySelector(".background");
  if (backgroundElement) {
    backgroundElement.insertAdjacentHTML("beforeend", snowFlake);
  }
}

function removeSnowLake() {
  const flakeWrappers = document.querySelectorAll(".flake-wrapper");
  const bodyHeight = document.documentElement.scrollHeight - 100;

  flakeWrappers.forEach((flakeWrapper) => {
    const flakePos = flakeWrapper.offsetTop;
